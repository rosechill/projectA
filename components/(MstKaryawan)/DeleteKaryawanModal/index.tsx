...
import { apiDeleteKaryawan } from "@/service/api/apiKaryawan";
...
const DeleteKaryawanModal: React.FC<DeleteModalProps> = ({
  isOpen,
  onClose,
  title,
  karyawanData,
  ...
  const handleSubmit = () => {
    ...
    apiDeleteKaryawan(karyawanDataHandler.id)
      .then(() => {
          ...
          <ModalBody>
            {karyawanData && (
              <div className="flex flex-col gap-2 pt-2">
                <h2>
                  Apakah ada yakin akan menghapus Karyawan
                  <span className="font-semibold">{karyawanData.id}</span> ?
                </h2>
              </div>
            )}
          </ModalBody>
